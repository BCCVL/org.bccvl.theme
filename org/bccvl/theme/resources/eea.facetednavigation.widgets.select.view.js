...
Faceted.SelectWidget.prototype = {
  ...
  synchronize: function(){
    ...
    jQuery.each(value, function(){
      var selectize = context.select[0].selectize;
      var is_selectize = typeof selectize !== 'undefined';
      if (!is_selectize) {
  ...
  count_update: function(data, sortcountable){
      ...
      if (typeof select[0].selectize === 'undefined') {
          ...
          var current_val = selectize.getValue();
          jQuery.each(options, function(key, opt) {
              opt.disabled = false;
              opt.count = data[opt.value] ? data[opt.value] : 0;
              if(!opt.count) {
                  opt.disabled = true;
              }
              selectize.updateOption(opt.value, opt);
          });
          if(sortcountable){
              selectize.settings.sortField = [
                  {'field': 'count',
                   'direction': 'desc'},
                  {'field': selectize.settings.labelField}
              ];
